(function(){
  // Songs page enhancements only if the container exists
  ...
  function render(songs){
    ...
    songs.forEach(s => {
      ...
      li.innerHTML = [
        '<div class="truncate"><span class="font-medium">', String(s.hymn_number), '</span> — ',
        String(s.title || ''), '</div>',
        '<div class="flex items-center gap-2">',
        '<button data-action="edit" data-id="', String(s.id), '" class="px-2 py-1 rounded-md border text-xs hover:bg-gray-100 dark:border-gray-700 dark:hover:bg-gray-700">Edit</button>',
        '<button data-action="delete" data-id="', String(s.id), '" class="px-2 py-1 rounded-md border text-xs text-red-600 border-red-300 hover:bg-red-50 dark:text-red-300 dark:border-red-700 dark:hover:bg-red-900/30">Delete</button>',
        '</div>'
      ].join('');
      list.appendChild(li);
    });
  ...
  list.addEventListener('click', async (e) => {
    ...
    if (btn.dataset.action === 'edit'){
      ...
      if (!values) return;
      const payload = { title: String(values.title || '').toUpperCase(), hymn_number: Number(values.hymn_number), misc: values.misc || null };
  ...
  if (form){
    form.addEventListener('submit', async (e) => {
      e.preventDefault();
      const data = {
        title: String(document.getElementById('title').value || '').toUpperCase(),
        hymn_number: parseInt(document.getElementById('hymn_number').value, 10),
      ...
      if (!response.ok) {
        ...
        UI.toast(err.detail || 'Error adding song', 'error');
        return;
      }
      form.reset();

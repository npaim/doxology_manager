    ...
    try {
      ...
      if (!Array.isArray(items) || items.length === 0){
        ...
        root.innerHTML = `<div class="text-sm text-gray-500">${(window.I18N ? I18N.t('no_items') : 'No items yet.')}
          <a class="underline" href="/services/new/choose?date=${d}">${(window.I18N ? I18N.t('new_service') : 'New Service')}</a>
        </div>`;
        return;
      }
      root.innerHTML = items.map(it => `
        <a href="/services/${it.id}" class="block rounded-md border border-gray-200 dark:border-gray-700 p-3 hover:bg-gray-50 dark:hover:bg-gray-800">
          <div class="text-sm font-medium">${it.leader || '-'}</div>
          <div class="text-xs text-gray-600 dark:text-gray-400 mt-1">${it.date}${it.time ? ' - ' + it.time : ''}</div>
          <div class="text-xs text-gray-600 dark:text-gray-400">${(window.I18N? I18N.t('preacher') : 'Preacher')}: ${it.preacher || '-'}</div>
          <div class="text-xs text-gray-600 dark:text-gray-400">${(window.I18N? I18N.t('sermon_title') : 'Sermon Title')}: ${it.title || '-'}</div>
        </a>
      `).join('');
    } catch(e){
